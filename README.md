...
4. The source repo contains a ready-made Jenkinsfile for scanning RailsGoat using Brakeman. Tell Jenkins about this by changing the `Build Configuration -> Script Path` field to `sec-tests/brakeman/Jenkinsfile`. When you hit the Save button, Jenkins should scan the repo, find the Jenkinsfile, and kick off the Brakeman static analysis code scan.
